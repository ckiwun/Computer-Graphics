...
void main()
{
	...
	vec3 obj_normal = mat3(c0,c1,c2) * normal;
	vec3 cube_coord = objectToWorld * obj_normal;
	...
	gl_FragColor = 0.5 * decal_color * (LMa + LMd * diffuse_term + LMs * pow(spec_term,shininess) ) + 0.6 * textureCube(envmap,cube_coord);
}
