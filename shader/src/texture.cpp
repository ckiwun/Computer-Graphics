...
{
    float3 normal;  
    int ip = i+1>=width? 0 : i+1;
    int im = i==0? width-1 : i-1;
    int jp = j+1>=height? 0 : j+1;
    int jm = j==0? height-1 : j-1;
    
    int h_ip = image[ip+j*width];
    int h_im = image[im+j*width];
    int h_jp = image[i+jp*width];
    int h_jm = image[i+jm*width];
    float diff_x = scale*(h_ip - h_im);
    float diff_y = scale*(h_jp - h_jm);
    normal = float3(diff_x/(510.0),diff_y/(510.0),1);
