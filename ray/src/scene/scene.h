...
public:
  ...
  bool intersect(ray& r, isect& i) const;
  bool kd_intersect(ray& r, isect& i) const;
  ...
  void buildKdTree(unsigned int d, unsigned int lsize);
...
public:
	...
	unsigned int target_lsize;
public:
	KdNode(BoundingBox& boundedbox,unsigned int d,unsigned int lsize):bbox(boundedbox),max_depth(d),target_lsize(lsize) {}
	~KdNode() {}
	...
	bool hit(ray& ,isect&,double&,double&);
