...
// previewview.cpp
//
// The code maintaining the preview view of the input images
//
...
void PreviewView::draw()
{
	...
	if ( m_pDoc->m_ucBitmap ) 
	{
		...
		//	printf("i,j = %d,%d",i,j);
		//	printf("(%d, ",m_pDoc->m_ucPreviewBackup[i*m_pDoc->m_nWidth+j]);
		//	printf("%d,  ",m_pDoc->m_ucPreviewBackup[i*m_pDoc->m_nWidth+j+1]);
		//	printf("%d)\n",m_pDoc->m_ucPreviewBackup[i*m_pDoc->m_nWidth+j+2]);
		//}
		...
		bitstart = m_pDoc->m_ucPreviewBackup + 3 * ((m_pDoc->m_nWidth * startrow) + scrollpos.x);
...
void PreviewView::resizeWindow(int	width, 
								int	height)
{
